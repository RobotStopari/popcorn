...
export default function AdminDeleteUsefulLinkDialog({ open, link, onClose, onConfirm }) {
  ...
  const handleConfirm = async () => {
    setDeleting(true);
    setError('');
    const ok = await onConfirm(link.id);
    setDeleting(false);
    if (ok) onClose();
    else setError(adminText('usefulLinks.deleteDialog.failed'));
  };
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="admin-delete-useful-link-title"
    >
      <div className="admin-modal__backdrop" onClick={onClose} aria-hidden="true" />
      <AdminModalPanel>
        <h2 id="admin-delete-useful-link-title" className="admin-modal__title">
          {adminText('usefulLinks.deleteDialog.title')}
        </h2>
        <p className="admin-modal__text">
          {adminText('usefulLinks.deleteDialog.body', { title: link.title })}
        </p>
