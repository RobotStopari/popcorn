...
export default function AdminDeletePublicationDialog({ open, publication, onClose, onConfirm }) {
  ...
  if (!mounted || !publication) return null;

  const handleConfirm = async () => {
    setDeleting(true);
    setError('');
    const ok = await onConfirm(publication.id);
    setDeleting(false);
    if (ok) onClose();
    else setError(adminText('publications.deleteDialog.failed'));
  };
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="admin-delete-publication-title"
    >
      <div className="admin-modal__backdrop" onClick={onClose} aria-hidden="true" />
      <AdminModalPanel>
        <h2 id="admin-delete-publication-title" className="admin-modal__title">
          {adminText('publications.deleteDialog.title')}
        </h2>
        <p className="admin-modal__text">
          {adminText('publications.deleteDialog.body', { title: publication.title })}
        </p>
